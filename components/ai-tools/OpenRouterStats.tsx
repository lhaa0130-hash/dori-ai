    ...
    <div className="rounded-2xl border border-neutral-200 dark:border-zinc-800 p-4 sm:p-5">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[15px] font-extrabold text-neutral-950 dark:text-white">전 세계 AI 모델 사용량 순위</h2>
        </div>
        <span className="text-[11px] text-neutral-400 whitespace-nowrap">{ago(s.updatedAt)}</span>
      </div>
      <p className="text-[11px] text-neutral-400 mb-4">OpenRouter 실사용 토큰 기준 · 6시간마다 갱신</p>

      {/* 사용량 순위 막대 */}
      <div className="space-y-1.5 mb-4">
        {s.topModels.slice(0, 8).map((m) => (
          <div key={m.rank} className="flex items-center gap-2">
            <span className="text-[11px] w-4 text-neutral-400 font-bold text-right">{m.rank}</span>
            <span className="text-[11px] w-[38%] truncate text-neutral-700 dark:text-neutral-300" title={m.name}>{m.name}</span>
            ...
            <span className="text-[10px] text-neutral-500 w-9 text-right tabular-nums">{m.share}%</span>
          </div>
        ))}
      </div>
      ...
      {s.providerShare?.length > 0 && (
        <div className="pt-3 border-t border-neutral-100 dark:border-zinc-800/60">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[11px] font-semibold text-neutral-500">제공사 점유율</span>
            {tr.length > 1 && (
              <span className="text-[10px] text-neutral-400">{s.providerTop} 추이</span>
            )}
          </div>
          {tr.length > 1 && (
          ...
          <div className="flex flex-wrap gap-1.5">
            {s.providerShare.map((p) => (
              <span key={p.provider} className="text-[11px] px-2 py-1 rounded-full bg-neutral-100 dark:bg-zinc-800 text-neutral-600 dark:text-neutral-300">
                {p.provider} <span className="font-bold text-neutral-900 dark:text-white">{p.pct}%</span>
              </span>
            ))}
          </div>
        </div>
