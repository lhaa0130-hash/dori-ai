    ...
    <div className="rounded-3xl border border-stone-200 dark:border-zinc-800 overflow-hidden">
      ...
      <div className="px-4 sm:px-5 pt-3 pb-2 bg-gradient-to-b from-[#F9954E]/10 to-transparent border-b border-stone-100 dark:border-zinc-800">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[16px] font-extrabold text-stone-950 dark:text-white">AI 모델 랭킹</h2>
            <span className="text-[10px] font-bold text-[#F9954E] border border-[#F9954E]/40 rounded-full px-1.5 py-0.5">LIVE</span>
          </div>
          <span className="text-[11px] text-stone-400 whitespace-nowrap">{s.total}개 모델</span>
        </div>
        <p className="text-[11px] text-stone-500 dark:text-stone-400 mt-0.5">전 세계 사용량 · 지능 · 가격을 한눈에</p>
      </div>
      ...
      <div className="grid grid-cols-3 divide-x divide-stone-100 dark:divide-zinc-800">
        ...
        <div className="px-3 sm:px-4 py-2">
          <p className="text-[11px] font-extrabold text-stone-700 dark:text-stone-200 mb-2">🔥 사용량</p>
        ...
        {/* 🧠 지능 */}
        <div className="px-3 sm:px-4 py-2">
          <p className="text-[11px] font-extrabold text-stone-700 dark:text-stone-200 mb-2">🧠 지능</p>
          <ol className="space-y-1.5">
            {intel.map((m, i) => {
        ...
        {/* 💰 가격 */}
        <div className="px-3 sm:px-4 py-2">
          <p className="text-[11px] font-extrabold text-stone-700 dark:text-stone-200 mb-2">💰 가격</p>
          <ol className="space-y-1.5">
            {price.map((m, i) => (
              <li key={m.name} className="flex items-center gap-1.5">
                <span className={`text-[11px] font-extrabold w-4 text-center shrink-0 ${i < 3 ? "text-[#F9954E]" : "text-stone-300 dark:text-stone-600"}`}>{i + 1}</span>
      ...
      <p className="px-4 sm:px-5 pb-2 text-[10px] text-stone-400 border-t border-stone-100 dark:border-zinc-800 pt-1.5">데이터: OpenRouter · 지능=Artificial Analysis · 가격=입력 토큰 100만 개당 USD</p>
    </div>
  );
}
