...
const AiToolsCard = React.memo(function AiToolsCard({ tool, rank }: AiToolsCardProps) {
  ...
  const borderStyle = useMemo(() => {
    if (rank === 1) return "border-yellow-400 dark:border-yellow-600 bg-yellow-50/30 dark:bg-yellow-900/10 shadow-sm";
    if (rank === 2) return "border-slate-300 dark:border-slate-500 bg-slate-50/30 dark:bg-slate-900/10 shadow-sm";
    if (rank === 3) return "border-orange-300 dark:border-orange-600 bg-orange-50/30 dark:bg-orange-900/10 shadow-sm";
    // 기본 테두리 유지 (호버 시 색상 변경 제거)
    return "border-strict bg-card hover:shadow-md";
  }, [rank]);
  ...
  const rankBadge = useMemo(() => {
    ...
    // 공통 뱃지 스타일 (작고 깔끔하게)
    // 공통 뱃지 스타일 (작고 깔끔하게, hover 시 확대)
    const badgeBase = "absolute -top-2 -left-2 z-[60] w-8 h-8 flex items-center justify-center rounded-full shadow-md border-2 border-white dark:border-[#222] transition-transform duration-300 hover:scale-110";

    if (r === 1)
      return (
        <div className={`${badgeBase} bg-gradient-to-br from-yellow-400 to-yellow-600 text-white`}>
          <Crown className="w-4 h-4" strokeWidth={2.5} />
        </div>
      );
    if (r === 2)
      return (
        <div className={`${badgeBase} bg-gradient-to-br from-slate-300 to-slate-500 text-white`}>
          <Medal className="w-4 h-4" strokeWidth={2.5} />
        </div>
      );
    if (r === 3)
      return (
        <div className={`${badgeBase} bg-gradient-to-br from-orange-300 to-orange-600 text-white`}>
          <Medal className="w-4 h-4" strokeWidth={2.5} />
        </div>
      );
    return null;
  ...
  return (
    <div className="relative group w-full h-full">
      ...
      <div
        ...
        className={`relative h-full flex flex-col rounded-lg border transition-all duration-300 overflow-hidden ${borderStyle}`}
      >
        {/* ⭐ 스포트라이트 효과 (테두리가 빛나는 효과) */}
        <div
