...
export default function BentoSection() {
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 sm:px-6 mb-12 sm:mb-24 animate-[fadeInUp_0.8s_ease-out_0.1s_forwards] opacity-0">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 auto-rows-[minmax(200px,auto)] md:auto-rows-[minmax(260px,auto)]">
        ...
        <Link href="/ai-tools" className={`md:col-span-2 md:row-span-2 ${cardClasses}`}
          ...
          <div className="relative z-10 flex flex-col justify-center h-full">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl flex items-center justify-center text-2xl md:text-3xl mb-4 md:mb-6 bg-white/20 backdrop-blur-md text-white shadow-inner border border-white/20">🏆</div>
            <h3 className="text-2xl md:text-4xl font-extrabold mb-2 md:mb-3 tracking-tight">{t.tools.title}</h3>
            {/* 👇 .ko 사용 */}
            <p className="text-blue-100 text-sm md:text-lg leading-relaxed whitespace-pre-line opacity-90">{t.tools.detail.ko}</p>
          </div>
          <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8 px-4 md:px-6 py-2 md:py-3 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white font-bold text-xs md:text-sm group-hover:bg-white group-hover:text-blue-600 transition-all flex items-center gap-2">
