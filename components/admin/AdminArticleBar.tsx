    ...
    <>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-[9998] flex items-center justify-center bg-black/60 dark:bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-stone-200 dark:border-stone-700">
              <div>
                <h2 className="text-foreground font-bold text-lg">✏️ 글 수정</h2>
                <p className="text-stone-500 dark:text-stone-400 text-sm mt-0.5 truncate max-w-md">{title}</p>
              </div>
              <button onClick={() => setShowEditModal(false)} className="text-stone-400 hover:text-foreground text-2xl leading-none px-2 transition-colors">×</button>
            </div>
            ...
            <div className="flex-1 overflow-hidden p-4">
              ...
              {loadingContent ? (
                <div className="flex items-center justify-center h-[55vh] text-stone-400">불러오는 중...</div>
              ) : (
                <textarea
                  value={editContent}
                  onChange={(e) => setEditContent(e.target.value)}
                  className="w-full h-full min-h-[55vh] bg-stone-50 dark:bg-stone-800 text-stone-900 dark:text-stone-100 text-sm font-mono p-4 rounded-xl border border-stone-200 dark:border-stone-600 focus:border-blue-500 focus:outline-none resize-none leading-relaxed"
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-stone-200 dark:border-stone-700">
              <span className="text-stone-400 dark:text-stone-500 text-xs">{editContent.length.toLocaleString()}자</span>
              <div className="flex gap-3">
                <button onClick={() => setShowEditModal(false)} disabled={loading} className="px-5 py-2 bg-stone-100 dark:bg-stone-700 hover:bg-stone-200 dark:hover:bg-stone-600 text-foreground text-sm rounded-xl transition-all">취소</button>
