...
export default function ProjectTopBar({ name, emoji }: { name: string; emoji?: string }) {
  return (
    <div className="fixed top-0 left-0 right-0 z-[10000] h-12 flex items-center gap-3 px-4 sm:px-6 bg-white/95 dark:bg-black/95 backdrop-blur border-b border-stone-200 dark:border-zinc-800">
      <Link
        href="/"
      ...
      <span className="text-[13px] font-bold text-stone-700 dark:text-stone-200 truncate">
        {emoji ? `${emoji} ` : ""}{name}
      </span>
    </div>
  );
