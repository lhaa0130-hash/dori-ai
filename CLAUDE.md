...
> 📌 **기준 문서**: 도구 라우팅·연결 상태·비용 원칙·업무 분담의 기준은
> Notion **「illo 컴퓨터 전달함 → AI 운영체계」**입니다. 저장소 문서와 어긋나면 **Notion을 따릅니다.**
> 두 총괄이사 간 소통도 **Notion 전달함**이 공식 채널입니다(GitHub 이슈 아님).
>
> 🏢 **조직 설계**: 두 총괄이사(컴퓨터 2대) 체제·본부 코드·저장소 소유권·자동화 배치는
> [`docs/ai-org-plan.md`](docs/ai-org-plan.md) 참조.
> 이 저장소는 **제1 총괄이사(1번 PC)** 담당이며, 산하에 `01-01 사이트 관리본부`와
> `01-02 상품 제작본부`가 있습니다.
>
> 💰 **비용**: API 비용 0원이 기본입니다. 유료 전환·자동 충전 금지, 유료 호출은 대표이사 사전 승인.
>
