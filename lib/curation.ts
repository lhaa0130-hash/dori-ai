...
const curationDirectory = path.join(process.cwd(), 'content/curation');
...
export function getCurationSlugs() {
  if (!fs.existsSync(curationDirectory)) return [];
  return fs.readdirSync(curationDirectory).filter(file => file.endsWith('.md'));
}

export function getCurationBySlug(slug: string): CurationPost | null {
  const realSlug = slug.replace(/\.md$/, '');
  const fullPath = path.join(curationDirectory, `${realSlug}.md`);
...
export function getAllCurations(): CurationPost[] {
  const slugs = getCurationSlugs();
  const trends = slugs
    .map((slug) => getCurationBySlug(slug))
    .filter((trend): trend is CurationPost => trend !== null)
    .sort((trend1, trend2) => {
      // 최신순으로 정렬 (날짜 내림차순)
