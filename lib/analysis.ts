...
export function getAnalysisSlugs() {
  if (!fs.existsSync(analysisDirectory)) return [];
  return fs.readdirSync(analysisDirectory).filter(file => file.endsWith('.md'));
}

export function getAnalysisBySlug(slug: string): AnalysisPost | null {
  const realSlug = slug.replace(/\.md$/, '');
  const fullPath = path.join(analysisDirectory, `${realSlug}.md`);
...
export function getAllAnalyses(): AnalysisPost[] {
  const slugs = getAnalysisSlugs();
  const trends = slugs
    .map((slug) => getAnalysisBySlug(slug))
    .filter((trend): trend is AnalysisPost => trend !== null)
    .sort((trend1, trend2) => {
      // 최신순으로 정렬 (날짜 내림차순)
