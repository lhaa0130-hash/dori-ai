...
| **BGM · 효과음 · 영상** | ⚠️ **미확정** | GPT 불가 · 힉스필드 보류 — Phase 0-6e에서 확정 |
...
| 정지 이미지 · 3D 레퍼런스 | **ChatGPT** | omnirouter | Gemini | ChatGPT 구독 |
| **영상** | ~~ChatGPT~~ **불가** | **omnirouter** | Gemini | ⚠️ 연결 미확인 |
| **음악 · 효과음** | ~~ChatGPT~~ **불가** | **omnirouter** | Gemini | ⚠️ 연결 미확인 |
| 3D 모델링 · 리깅 · 애니메이션 | **Meshy** | — | — | 구독 예정 (**사실상 필수**) |

> ⚠️ **GPT는 영상·음악을 생성하지 못한다.** 정지 이미지만 가능하다.
> 따라서 영상·음악은 대표가 정한 1순위가 처음부터 성립하지 않고, **실질 1순위가 omnirouter**다.
>
> ⛔ **힉스필드는 당분간 사용하지 않는다.** 커넥터는 연결돼 있지만 미디어 스택에서 뺀다.
> **힉스필드는 omnirouter와 별개 서비스이며 2순위가 아니다** — 이 문서 초판이 둘을 혼동했다(10절).
...
힉스필드 커넥터는 연결돼 있다. 제공 범위는 `generate_image` · `generate_video` ·
`generate_audio` · **`generate_3d`**(이미지 → 3D GLB 메시)와 업스케일·배경 제거·모션 컨트롤 등이다.

**그러나 대표 결정으로 당분간 쓰지 않는다.** 연결은 유지하되 미디어 스택에서 뺀다.
다시 쓰기로 하면 되돌리면 되고, 커넥터를 해제할 필요는 없다.

> ⚠️ **정정 — 힉스필드는 omnirouter가 아니다.**
> 이 문서 초판은 대표가 말한 2순위 도구(당시 "omnipotent"로 잘못 옮겨 적었고 실제 이름은
> **omnirouter**)를 힉스필드와 동일시했다. **둘은 별개 서비스다.**
> 힉스필드는 우선순위 목록에 원래 없던 도구이고, 2순위 자리는 omnirouter의 것이다.

### ⚠️ 미디어 스택의 공백 — 영상·음악

두 가지가 겹쳐 **영상과 음악을 만들 수단이 현재 확정돼 있지 않다.**

| 사실 | 출처 |
|---|---|
| **GPT는 영상·음악을 생성하지 못한다** (정지 이미지만 가능) | 대표 확인 |
| 힉스필드는 영상·오디오를 생성하지만 **보류** | 대표 결정 |
| **omnirouter의 연결 여부·제공 범위 미확인** | 이 세션에서 확인 불가 |

즉 영상·음악은 **1순위가 처음부터 성립하지 않고 2순위(omnirouter)가 실질 1순위**인데,
그 omnirouter가 아직 붙어 있지 않다.

> 🎮 **게임 개발에 직결되는 공백이다.** BGM·효과음 없이는 게임을 완성할 수 없으므로,
> Phase 2(게임) 착수 전에 반드시 메워야 한다. 선택지는 셋 —
> ① omnirouter 연결 ② Gemini로 내려가기 ③ 힉스필드 보류 해제.
> Phase 0-6e 항목으로 관리한다.

### 3D 경로가 Meshy 하나만 남았다

이전에는 "힉스필드 `generate_3d`로 먼저 시험해보면 Meshy 구독이 불필요할 수도 있다"는
선택지가 있었다. **힉스필드 보류로 그 선택지가 사라졌으므로, 게임 3D 에셋이 필요해지는 시점에
Meshy 구독은 사실상 확정이다.** Phase 2 비용에 반영한다.
...
| **omnirouter** | **연결 여부와 제공 범위(영상·음악 생성 가능한지).** 영상·음악의 실질 1순위이므로 게임 개발의 선결 조건이다 |
...
| 0-6e | **영상·음악 수단 확정**(omnirouter 연결 / Gemini / 힉스필드 해제 중 택1) | 영상 1개 + 효과음 1개를 실제로 생성 성공 |
