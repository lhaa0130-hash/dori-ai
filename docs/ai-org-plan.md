...
| 단계 | 담당 | 도구 |
|---|---|---|
| 기획 · 시나리오 · 밸런싱 문서 | 기획담당 | OpenAI |
| 캐릭터·배경 컨셉 이미지 | 이미지생성 | OpenAI → omnipotent → Gemini |
| 3D 모델링 · 리깅 · 애니메이션 | 3D담당 | Meshy |
| BGM · 효과음 | 오디오생성 | OpenAI → omnipotent → Gemini |
...
| 기획 · 문서 | **OpenAI** | — | — | Pro 구독 중 |
| 이미지 · 영상 · 음악 | **OpenAI** | omnipotent | Gemini | OpenAI는 구독, 나머지 API 종량 |
| 3D 모델링 · 리깅 · 애니메이션 | **Meshy** | — | — | 구독 예정 |

**폴백 규칙**: 1순위가 실패하거나(할당량 초과, 품질 미달) 해당 기능을 지원하지 않을 때만
2순위로 내려간다. 비용이 아니라 **가용성과 품질**이 폴백 사유다.

**구독 현황**: 클로드코드 Max · OpenAI Pro 구독 중 / Meshy 구독 예정 /
그 외는 구독 계획 없음, 필요한 것은 API 종량으로 사용.
...
| 항목 | 확인할 것 |
|---|---|
| **OpenAI MCP** | **현재 연결 수단이 전혀 없다.** 커넥터 목록에도 없고 `OPENAI_API_KEY` 환경변수도 없다. Pro 구독 중이므로 (a) OpenAI MCP 서버 설치 (b) 별도 API 키 발급 후 종량 사용 중 택1이 필요하다. **9절의 "기획·문서 = OpenAI" 배정은 이것이 해결돼야 실행 가능하다** |
...
| 0-6 | 두 PC에 `.mcp.json` 배치, MCP 3종 확인(10절) | 확인 필요 표가 전부 채워짐 |
