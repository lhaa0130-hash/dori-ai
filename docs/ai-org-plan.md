...
|---|---|---|---|---|
| 총괄 · 개발 | **클로드코드** | — | — | Max 구독 |
| 기획 · 문서 | **OpenAI** | — | — | ChatGPT 구독 (MCP로 연결) |
| 이미지 · 영상 · 음악 | **OpenAI** | 힉스필드 | Gemini | 양쪽 다 구독 |
...
**주력 도구는 전부 구독으로 덮인다.** 클로드코드 Max, ChatGPT, 힉스필드 모두 구독 중이고,
OpenAI는 **MCP를 통해 구독 그대로 쓰고 있다** — 2번 PC에서 이미지 생성까지 확인됐다(10절).
별도 API 키를 발급하지 않았다.

종량 과금이 남아 있는 항목은 다음뿐이다.

| 항목 | 성격 | 비고 |
|---|---|---|
| fal | 종량 | 동물도감 이미지 — 현재 잔액 부족으로 255종에서 정지 |
| Meshy | 구독 예정 | 힉스필드 `generate_3d` 선시험 결과에 따라 불필요할 수도 |
...
**구독 현황**: 클로드코드 Max · ChatGPT · 힉스필드 구독 중 / Meshy 구독 예정 /
그 외 구독 계획 없음.
...
| 힉스필드 | 이미지·영상·오디오·3D 메시 | ✅ 연결됨 |
| `meshy` | 3D 리깅·애니메이션 | Phase 2 (선시험 후 판단) |
...
대표가 "omnipotent"라고 불렀던 2순위 미디어 도구의 실체가 힉스필드다. 제공 범위:
