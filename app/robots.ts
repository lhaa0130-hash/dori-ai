...
export default function robots(): MetadataRoute.Robots {
  ...
  const disallow = [
    ...
    "/profile",
    // ⚠️ 2026-08-13 — My World 를 /@핸들 공개 홈에 연결하면서 셋을 함께 막는다.
    //    · /my-world : 회원 전용 편집기. 로그아웃 크롤러에겐 "로그인하면 …" 한 줄뿐이다.
    //    · /at       : /@* rewrite 의 내부 렌더러. 이 URL 자체는 사람이 볼 화면이 아니다.
    //    · /@        : 공개 프로필 본체. **지금 막는 이유가 핵심이다** —
    //      회원 7명·피드 글 2개 상태라 프로필 대부분이 기본 방·빈 소개다. 이걸 색인시키면
    //      "얇은 페이지 다수"라는, 지금까지 세 번 지적받은 그 모양이 그대로 재현된다.
    //      기능은 지금 만들어 두되 공개면은 승인 뒤에 연다(공개면 변화 0 원칙).
    //    ▶ 애드센스 승인 후 + 프로필에 실제 내용이 쌓인 뒤 "/@" 와 "/at" 를 여기서 빼라.
    //      그때 sitemap.ts 에 /@핸들 을 넣을지도 함께 판단한다(지금은 넣지 않는다).
    "/my-world",
    "/at",
    "/@",
    "/messages",
