    ...
    <main className="w-full min-h-screen max-w-5xl mx-auto px-4 pt-8 pb-20">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div>
          ...
          <h1 className="text-[26px] sm:text-[32px] font-extrabold text-neutral-950 dark:text-white mt-2 break-keep">🖼️ 친구들이 만든 동물</h1>
          <p className="text-[14px] text-neutral-400 dark:text-neutral-500 break-keep">회원들이 상상해서 만든 나만의 동물들이에요.</p>
        </div>
      ...
      {items === null ? (
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {Array.from({ length: 8 }).map((_, i) => <div key={i} className="rounded-2xl aspect-[4/5] bg-neutral-100 dark:bg-zinc-900 animate-pulse" />)}
        </div>
      ) : items.length === 0 ? (
        <div className="mt-16 text-center">
          <p className="text-[15px] font-bold text-neutral-500">아직 만들어진 동물이 없어요.</p>
        ...
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {items.map((c) => (
            <div key={c.id} className="group text-left bg-white dark:bg-zinc-900 border border-neutral-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
              <button onClick={() => setDetail(c)} className="block w-full">
                <div className="relative aspect-[4/5] overflow-hidden bg-neutral-100 dark:bg-zinc-900">
                ...
                <div className="p-2.5">
                  <div className="font-bold text-[13px] text-neutral-900 dark:text-white truncate">{c.animal_name}</div>
                  <div className="text-[11px] text-neutral-400 truncate">by {c.authorName}</div>
                </div>
              </button>
              <button onClick={() => toggleLike(c)} className="w-full flex items-center justify-center gap-1 pb-2 text-[12px] font-bold transition" style={{ color: c.likedByMe ? "#F9954E" : undefined }}>
                <span className={c.likedByMe ? "" : "text-neutral-400"}>{c.likedByMe ? "❤️" : "🤍"}</span>
                <span className={c.likedByMe ? "text-[#F9954E]" : "text-neutral-400"}>{c.likeCount}</span>
              </button>
            </div>
          ))}
        </div>
          ...
          <div className="relative w-full max-w-sm max-h-[88vh] overflow-y-auto rounded-3xl bg-white dark:bg-zinc-950 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4">
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-lg font-extrabold text-neutral-900 dark:text-white break-keep">{detail.animal_name}</h3>
              ...
              <p className="text-[12px] text-neutral-400">by {detail.authorName}</p>
              ...
              {detail.kid_friendly_desc && <p className="text-[13px] text-neutral-600 dark:text-neutral-300 mt-2 leading-relaxed break-keep">{detail.kid_friendly_desc}</p>}
              ...
              {detail.facts?.length > 0 && (
                <ul className="mt-3 space-y-1">
                  {detail.facts.map((f, i) => <li key={i} className="text-[12px] text-neutral-500 dark:text-neutral-400 pl-3 relative break-keep before:content-['•'] before:absolute before:left-0 before:text-[#F9954E]">{f}</li>)}
                </ul>
              )}
              <div className="mt-4 flex items-center gap-2">
                <button onClick={() => toggleLike(detail)} className="flex-1 rounded-2xl border border-neutral-200 dark:border-zinc-800 py-2.5 text-[13px] font-bold transition" style={{ color: detail.likedByMe ? "#F9954E" : undefined, borderColor: detail.likedByMe ? "#F9954E" : undefined }}>
