...
const SECTIONS = [
  ...
  // 커뮤니티는 아직 DB 연동이 없는 스텁이라 홈에서 링크하지 않는다(로그아웃 방문자에겐 빈 화면)
  .filter((s) => SHOW_COMMUNITY || s.href !== "/community")
  // 프로젝트는 '운영 중 0개' 상태라 홈에서 링크하지 않는다
  .filter((s) => SHOW_PROJECTS || s.href !== "/projects");
          ...
          <div className="flex gap-1 w-max">
            {SECTIONS.map(({ label, href, Icon }) => (
              <Link
                key={href}
                href={href}
                className="flex flex-col items-center gap-1.5 py-2 w-[64px] shrink-0 rounded-2xl active:bg-stone-50 dark:active:bg-zinc-900 transition-colors"
              >
