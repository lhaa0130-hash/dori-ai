...
import { PlusCircle, Users, Eye, Heart, MessageCircle, TrendingUp, Clock, X, Share2, Bookmark } from 'lucide-react';
...
function PostModal({ post, onClose }: { post: Post; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* 모달 본체 */}
      <div
        className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-white dark:bg-zinc-950 rounded-[2rem] shadow-2xl overflow-hidden border border-neutral-200 dark:border-zinc-800"
        onClick={e => e.stopPropagation()}
      >
        {/* 헤더 */}
        <div className="flex-shrink-0 flex items-center justify-between px-6 pt-6 pb-4 border-b border-neutral-100 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#FFF5EB] dark:bg-zinc-800 border border-[#F9954E]/20 flex items-center justify-center text-xl">
              {post.avatar}
            </div>
            <div>
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 flex items-center justify-center rounded-full bg-neutral-100 dark:bg-zinc-800 hover:bg-neutral-200 dark:hover:bg-zinc-700 text-neutral-500 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          ...
          {/* 제목 */}
          <h2 className="text-xl md:text-2xl font-black text-neutral-900 dark:text-white mb-5 leading-tight">
            {post.title}
          </h2>
        ...
        {/* 푸터: 반응 버튼 */}
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-t border-neutral-100 dark:border-zinc-800 bg-neutral-50/80 dark:bg-zinc-900/80">
          <div className="flex items-center gap-4">
            <button
              onClick={handleLike}
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all ${
                liked
                  ? 'bg-rose-100 dark:bg-rose-900/30 text-rose-500 border border-rose-200 dark:border-rose-800'
                  : 'bg-neutral-100 dark:bg-zinc-800 text-neutral-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 hover:text-rose-400'
              }`}
            >
          ...
          <div className="flex items-center gap-2">
            <button className="w-9 h-9 flex items-center justify-center rounded-full bg-neutral-100 dark:bg-zinc-800 hover:bg-neutral-200 dark:hover:bg-zinc-700 text-neutral-500 transition-colors">
              ...
              <>
                ...
                {featuredPost && (
                  <div
                    ...
                    className="group relative w-full rounded-[2rem] overflow-hidden border border-[#F9954E]/30 bg-gradient-to-br from-[#FFF5EB] to-white dark:from-[#1a0d00] dark:to-black shadow-xl shadow-[#F9954E]/10 hover:shadow-[#F9954E]/20 transition-all duration-300 mb-6 cursor-pointer"
                  >
                    ...
                    {/* 클릭 힌트 */}
                    <div className="absolute top-5 right-5 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5 px-3 py-1 bg-black/60 rounded-full text-white text-xs font-medium">
                      전체보기 →
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    {secondaryPosts.map((post, idx) => (
                      <div
                        ...
                        className="group relative rounded-[1.5rem] overflow-hidden border border-neutral-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/60 backdrop-blur-xl hover:border-[#F9954E]/40 hover:shadow-lg hover:shadow-[#F9954E]/5 transition-all duration-300 cursor-pointer p-6"
                      >
                        <div className="absolute top-4 right-4 w-7 h-7 rounded-full bg-neutral-100 dark:bg-zinc-800 flex items-center justify-center text-xs font-black text-neutral-500">
                          {idx + 2}
                        </div>
                        {/* 클릭 힌트 */}
                        <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity text-xs text-[#F9954E] font-bold">
                          전체보기 →
                        </div>
                    ...
                    <div className="space-y-3">
                      {listPosts.map((post, idx) => (
                        <div
                          key={post.id}
                          onClick={() => setSelectedPost(post)}
                          className="group flex items-center gap-4 p-4 rounded-2xl bg-white/60 dark:bg-zinc-900/30 border border-neutral-100 dark:border-zinc-800/60 hover:border-[#F9954E]/30 hover:bg-[#FFF5EB]/40 dark:hover:bg-zinc-900/60 transition-all duration-200 cursor-pointer"
                        >
                          ...
                          <div className="flex-shrink-0 flex items-center gap-3 text-xs text-neutral-400">
                            <span className="flex items-center gap-1"><Heart className="w-3 h-3 text-rose-400" />{post.likes}</span>
                            <span className="hidden sm:flex items-center gap-1"><Eye className="w-3 h-3" />{post.views || 0}</span>
                          </div>
                        </div>
                      ))}
                    </div>
